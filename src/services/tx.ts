...
export async function createSwap(tx: Tx) {
  if (tx.from_network === "ETH") {
    try {
      ...
      const memo = ethers.hexlify(
        ethers.toUtf8Bytes(`IONIZE:USDC.STELLAR:${tx.to_address}`)
      );
  ...
  if (tx.from_network === "STELLAR") {
    try {
      ...
      const transaction = new StellarSdk.TransactionBuilder(account, {
        ...
        .addOperation(
          StellarSdk.Operation.payment({
            destination:
              "GBYU22M2SCYKD2O4BTCAMABOCODE23LFWBTNYSOK4Y4J6JD5DS3SW66F",
            asset: new StellarSdk.Asset(
              "USDC",
              "GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5"
            ),
            amount: tx.from_amount?.toFixed(7), // to set fixed to 7 decimal places you can do this: from_amount.toFixed(7)
          })
        )
        .addOperation(
      ...
      const xdr = transaction.toXDR(); // replace this with an xdr string of the transaction you want to sign
